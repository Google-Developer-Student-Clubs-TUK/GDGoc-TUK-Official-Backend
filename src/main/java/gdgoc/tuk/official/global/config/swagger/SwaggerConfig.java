...
@OpenAPIDefinition(
    info =
        @Info(
            title = "Google Developers Group TUK Official",
            description = "GDGoC TUK 공식 홈페이지 API Docs",
            version = "v1"))
  ...
  private <T> Schema<T> wrapSchema(
    ...
    for (Field field : type.getDeclaredFields()) {
      ...
      if (field.getName().equals("httpStatusCode")) {
        wrapperSchema.addProperty(field.getName(), new Schema<>().type("integer").example(code));
      }
